...
#define FILENAME "write_heavy_file"
#define NUM_PAGES 100
#define NUM_WRITES (NUM_PAGES * 100) /* 100 writes per page */
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < NUM_PAGES; i++) {
    ...
    *((int *)buf) = 0; /* Write 0 as initial content */
  ...
  /*
   * ==========================================================
   * PHASE 2: TEST (Read, modify, and mark dirty)
   * ==========================================================
   */
  if ((fd = PF_OpenFile(FILENAME)) < 0) {
    PF_PrintError("open file for write test");
    exit(1);
  }

  Q_PRINTF("Performing %d random writes (read-modify-write)...\n", NUM_WRITES);
  for (i = 0; i < NUM_WRITES; i++) {
    int page_to_write = rand() % NUM_PAGES;
    if ((error = PF_GetThisPage(fd, page_to_write, &buf)) != PFE_OK) {
      PF_PrintError("get this page");
      exit(1);
    }
    
    /* Modify the page content */
    *((int *)buf) += 1; /* Increment the value */

    /* Unfix as *DIRTY* */
    if ((error = PF_UnfixPage(fd, page_to_write, TRUE)) != PFE_OK) {
      PF_PrintError("unfix read page");
      exit(1);
    }
  }

  /* This close will force all dirty pages to be written to disk */
  Q_PRINTF("Closing file, flushing all dirty pages...\n");
  if ((error = PF_CloseFile(fd)) != PFE_OK) {
    PF_PrintError("close file after write test");
    exit(1);
  }
  ...
  if (g_quiet) {
    printf("WriteHeavy,%s,", strategy_name); /* Print "WriteHeavy,LRU," */
    PF_PrintStats();                         /* Print "logical,physical,writes\n" */
  } else {
    printf("\n--- Final Statistics (Write-Heavy) ---\n");
    PF_PrintStats();
  }
